...
import { fetchAllTransaction } from '../redux/transactionSlice';


function Transactions() {
  const [sort, setSort] = useState('recent')
  const allTransaction = useSelector(state => state.transactionReducer.allTransaction)
  console.log(allTransaction)
  const dispatch = useDispatch();

  useEffect(() => {
    dispatch(fetchAllTransaction(sort))
  }, [sort])

  const showAllTransactions = allTransaction.map((item) => {
    return (
